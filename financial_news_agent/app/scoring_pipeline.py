...
Modes:
  - Single query (Argo fan-out):  python scoring_pipeline.py --query-index 0
  - All queries (standalone):     python scoring_pipeline.py
  - Subset:                       python scoring_pipeline.py --query-index 0 --query-index 3

Exit codes:
  0 = success (articles inserted or all duplicates — normal operation)
  1 = fatal error (missing config, connection failure, zero results with zero duplicates)
  2 = partial failure (>50% LLM errors within the processed articles)
"""
...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for Argo fan-out and standalone modes."""
    parser = argparse.ArgumentParser(description="Financial news scoring pipeline")
    parser.add_argument(
        "--query-index",
        type=int,
        action="append",
        default=None,
        help="Index of search query to run (from config search_queries). "
             "Can be specified multiple times. If omitted, runs all queries.",
    )
    parser.add_argument(
        "--max-results",
        type=int,
        default=None,
        help="Override tavily.max_results (capped at 20 by Tavily API).",
    )
...
def load_config() -> dict:
    ...
    logger.info(
        "Config loaded: %d search_queries, %s symbols",
        len(config.get("search_queries", [])),
        config.get("symbols"),
    )
    return config
...
def _is_retryable_llm_error(exc: Exception) -> bool:
    """True if the LLM/API error is transient (rate limit, server error, timeout, connection)."""
    code = getattr(exc, "status_code", None) or getattr(exc, "code", None)
    if code is not None:
        if code == 429:
            return True
        if 500 <= (code if isinstance(code, int) else 0) <= 504:
            return True
...
def _extract_json_from_llm_text(text: str) -> str | None:
    ...
    # Fall back to first {...} object (non-greedy to avoid matching across multiple objects)
    ...
    if brace:
        ...
        try:
            ...
            # Greedy fallback for nested objects
            brace_greedy = re.search(r"\{[\s\S]*\}", text)
            if brace_greedy:
                return brace_greedy.group(0).strip() or None
    return text if text.startswith("{") else None


def _message_text(raw: str | list | dict | None) -> str:
    """Extract text content from various LLM message formats."""
    if raw is None:
        return ""
...
def _select_queries(config: dict, args: argparse.Namespace) -> list[str]:
    """Select which search queries to run based on CLI args."""
    all_queries = config.get("search_queries", ["USDJPY news", "EURUSD news"])

    if args.query_index is not None:
        # Argo fan-out mode: run specific query indices
        selected = []
        for idx in args.query_index:
            if 0 <= idx < len(all_queries):
                selected.append(all_queries[idx])
            else:
                logger.warning("--query-index %d out of range (0-%d), skipping", idx, len(all_queries) - 1)
        if not selected:
            logger.error("No valid query indices provided. Available: 0-%d", len(all_queries) - 1)
            sys.exit(1)
        return selected

    # Standalone mode: respect MAX_QUERIES_PER_CYCLE env
...
def run_one_cycle(
    ...
    """Run scoring pipeline for the given queries.

    Returns exit code: 0=success, 1=fatal, 2=partial failure.
    """
    ch_db = config.get("clickhouse", {})
    db_name = os.environ["CLICKHOUSE_DATABASE"]
    table_news = ch_db.get("table_news_items", "fn_news_items")
    ch_settings: dict = {}
    if ch_db.get("max_memory_usage_bytes") is not None:
        ch_settings["max_memory_usage"] = int(ch_db["max_memory_usage_bytes"])
    ...
    logger.info(
        "ClickHouse connected: %s:%s/%s (table=%s)",
        secrets["CLICKHOUSE_HOST"],
        secrets["CLICKHOUSE_PORT"],
        db_name,
        table_news,
    )
    ...
    logger.info("Running %d queries", len(queries_to_run))

    try:
        for query in queries_to_run:
            exclude_domains = tavily_cfg.get("exclude_domains") or []
            effective_max_results = min(
                max_results_override or tavily_cfg.get("max_results", 30),
                20,  # Tavily API hard cap
            )
            logger.info(
                "Tavily search: query=%r max_results=%d include_domains=%d exclude_domains=%d",
                query,
            ...
            for i, r in enumerate(to_process):
                ...
                existing = ch_client.query(
                    ...
                    settings=ch_settings if ch_settings else None,
                )
                ...
                try:
                    ...
                    logger.info("LLM scoring: model=%s prompt_len=%d", model, len(prompt))

                    # Build completion kwargs from config for tunable performance.
                    create_kwargs = {
                        "model": model,
                    ...
                    if novita_cfg.get("timeout") is not None:
                        create_kwargs["timeout"] = float(novita_cfg["timeout"])
                    if novita_cfg.get("top_p") is not None:
                        create_kwargs["top_p"] = float(novita_cfg["top_p"])
                    if novita_cfg.get("frequency_penalty") is not None:
                        create_kwargs["frequency_penalty"] = float(novita_cfg["frequency_penalty"])
                    if novita_cfg.get("presence_penalty") is not None:
                        create_kwargs["presence_penalty"] = float(novita_cfg["presence_penalty"])
                    if novita_cfg.get("response_format_json") is True:
                        create_kwargs["response_format"] = {"type": "json_object"}

                    max_retries = max(0, int(novita_cfg.get("max_retries", 2)))
                    for attempt in range(max_retries + 1):
                        try:
                            resp = client_llm.chat.completions.create(**create_kwargs)
                            break
                        except Exception as e:
                            if attempt < max_retries and _is_retryable_llm_error(e):
                                backoff = (2 ** attempt) * 2
                                logger.warning("LLM attempt %d failed (retryable), retrying in %ds: %s", attempt + 1, backoff, e)
                                time.sleep(backoff)
                            else:
                                raise
                    ...
                    llm_errors += 1
                    logger.warning("LLM JSON parse failed for %s url=%s: %s", article_id, url[:60], e)
                    scores = {"summary": "", "relevance_score": 0}
                except Exception as e:
                    llm_errors += 1
                ...
                if ch_settings:
                    ch_client.insert(table_news, [row], column_names=col_names, settings=ch_settings)
                else:
                    ch_client.insert(table_news, [row], column_names=col_names)
                inserted += 1
        ...
        ch_client.close()
        logger.info(
            "Cycle summary: inserted=%d skipped_duplicate=%d skipped_no_content=%d "
            "skipped_no_primary_currency=%d llm_errors=%d total_processed=%d",
            inserted, skipped_duplicate, skipped_no_content,
            skipped_no_primary_currency, llm_errors, total_articles_processed,
        )

    # Determine exit code for Argo
    if total_articles_processed == 0 and skipped_duplicate == 0:
        logger.error("No articles processed and no duplicates found — possible Tavily or config issue")
        return 1
    if total_articles_processed > 0 and llm_errors > (total_articles_processed * 0.5):
        logger.warning("More than 50%% of articles had LLM errors (%d/%d)", llm_errors, total_articles_processed)
        return 2
    return 0
...
def main() -> None:
    args = parse_args()
    logger.info("financial_news_scoring pipeline starting")
    config = load_config()
    config_dir = Path(__file__).resolve().parent
    secrets = get_secrets_from_env()
    logger.info(
        "Secrets loaded: TAVILY_API_KEY=%s NOVITA_API_KEY=%s CLICKHOUSE=%s",
        "***" if secrets.get("TAVILY_API_KEY") else "MISSING",
        "***" if secrets.get("NOVITA_API_KEY") else "MISSING",
        f"{secrets.get('CLICKHOUSE_HOST')}:{secrets.get('CLICKHOUSE_PORT')}",
    )

    queries_to_run = _select_queries(config, args)
    logger.info("Queries to run (%d): %s", len(queries_to_run), queries_to_run)
    ...
    logger.info("Cycle done. exit_code=%d", exit_code)
    sys.exit(exit_code)
