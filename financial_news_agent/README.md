...
| File | Purpose |
|------|---------|
| `config.yaml` | Non-secret config (symbols, queries, decay params, table names, optional `clickhouse.max_memory_usage_bytes`). |
| `docs/clickhouse-doks-tuning.md` | Query DOKS cluster and tune ClickHouse memory (e.g. after MEMORY_LIMIT_EXCEEDED). |
...
| `docker-compose.yml` | Local run: financial-news-scoring-api + ClickHouse. |
...
- `CLICKHOUSE_USERNAME` – e.g. `financial_news_scoring`
- `CLICKHOUSE_PASSWORD` – same value used when running `init_clickhouse.sh`
- `CLICKHOUSE_DATABASE` – e.g. `financial_news_scoring`
...
```bash
cd src/financial_news_scoring_api
...
cd src/financial_news_scoring_api
cp env.example .env
# Edit .env: set TAVILY_API_KEY, NOVITA_API_KEY, CLICKHOUSE_PASSWORD (and optionally CLICKHOUSE_USERNAME=financial_news_scoring)
...
### Remote / managed ClickHouse (DOKS, operator)

On managed ClickHouse, **CREATE USER** is often disabled (users are created by the operator). If `init_clickhouse.sh` fails with HTTP 500 on the first statement, skip user/database creation and run only table/column scripts:

```bash
doppler run -- env SKIP_CREATE_USER_DATABASE=1 ./scripts/init_clickhouse.sh
```

Ensure the `financial_news_scoring` user and database already exist (e.g. created by the ClickHouse operator or control plane).

### Run pipeline (one shot)

```bash
docker compose run --rm financial-news-scoring-api
```
...
```bash
docker compose run --rm -e CRON_INTERVAL_MINUTES=15 financial-news-scoring-api
```

Or in `.env`: `CRON_INTERVAL_MINUTES=15`, then `docker compose run --rm financial-news-scoring-api`.
...
```bash
docker build -t financial-news-scoring-api .
docker run --rm \
...
```bash
./doppler/run_api.sh
# or: doppler run --project financial_news_scoring_api --config dev -- uvicorn api:app --host 0.0.0.0 --port 8000
```
